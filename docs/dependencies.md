...
| `HOST`, `PORT` | lidas por `scripts/start.sh`/`scripts/restart.sh` (não pelo Python) | Endereço/porta do uvicorn ao subir a aplicação web; padrão `0.0.0.0:8000` (todas as interfaces). |
| `MAX_UPLOAD_BYTES` | `backend/src/routes/api.py`, lida no import do módulo | Teto de tamanho por arquivo em `POST /api/jobs`; acima disso o arquivo é rejeitado (o upload é abortado e o arquivo parcial apagado *durante* a escrita, não depois de gravar tudo — ver `_gravar_upload_com_teto`). Padrão `500000000` (500 MB) — calibrado para manuais/documentação de fornecedor reais, que chegam a centenas de MB. |
...
| `MAX_UPLOAD_PAGES` | `backend/src/services/motor_pool.py`, lida no import do módulo | Vira `Config.max_pages` do motor da aplicação web (a CLI não usa esta variável — lá quem decide é `--max-pages`, desligado por padrão). Padrão `10000` — não é mais política de uso (manuais reais de 1000-2000 páginas são comuns), é só proteção contra entrada patológica; a mensagem de rejeição nomeia as duas variáveis (`--max-pages`/`MAX_UPLOAD_PAGES`) porque `converter_arquivo()` não sabe qual interface a chamou. |
| `AVISO_ESTIMATIVA_MINUTOS` | `backend/src/routes/api.py`, lida no import do módulo | Limiar (minutos) acima do qual o banner de aviso de estimativa aparece na UI; exposto ao frontend via `aviso_estimativa_minutos` na resposta de `GET /api/jobs`. A estimativa de duração em si (`Job.estimativa_segundos`) é sempre exibida na linha do job, independente deste limiar — só o banner é condicional. Padrão `30`. |
